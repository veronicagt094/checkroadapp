...
export default function Boton() {
  const mock = ["List1", "List2", "List3", "List4"];
  const [selectedMock, setSelectedMock] = useState("List1");
  return (
    <div>
      <Listbox as="div" value={selectedMock} onChange={setSelectedMock}>
        {({ open }) => (
          <>
            ...
            <div className="relative">
              <span className="inline-block w-full">
                <Listbox.Button className="pl-3 py-2 w-full text-left focus:outline-none focus:shadow-outline-blue focus:border-blue-300 relative border shadow-sm border-gray-300 rounded text-gray-800">
                  <span className="block truncate">{selectedMock}</span>
                </Listbox.Button>
              </span>
              <Transition
                show={open}
              ...
              >
                ...
                >
                  {mock.map((mock) => (
                    <Listbox.Option key={mock} value={mock}>
                      {({ selected, active }) => (
                        <div
                          className={`${
                            active
